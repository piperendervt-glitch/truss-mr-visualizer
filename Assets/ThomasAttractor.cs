...
public class ThomasAttractor : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        for (int step = 0; step < stepsPerFrame; step++)
        {
            float rdx = Mathf.Sin(refY) - b * refX;
            float rdy = Mathf.Sin(refZ) - b * refY;
            float rdz = Mathf.Sin(refX) - b * refZ;
            refX += rdx * dt; refY += rdy * dt; refZ += rdz * dt;
        }
        float sepX = refX - tx, sepY = refY - ty, sepZ = refZ - tz;
        ...
        if (d1 > 0f)
        {
            ...
            lyapFrames++;
            float s = lyapD0 / d1;
            refX = tx + sepX * s; refY = ty + sepY * s; refZ = tz + sepZ * s;
        }
    ...
    void UpdateParticles(Vector3 center, Quaternion rot)
    {
        for (int i = 0; i < particleCount; i++)
        {
            ...
            for (int step = 0; step < stepsPerFrame; step++)
            {
                float dxdt = Mathf.Sin(cy) - b * cx;
                float dydt = Mathf.Sin(cz) - b * cy;
                float dzdt = Mathf.Sin(cx) - b * cz;
            ...
            Vector3 localPos = new Vector3(cx, cy, cz) * thomasScale;
            Vector3 worldPos = center + rot * localPos;
